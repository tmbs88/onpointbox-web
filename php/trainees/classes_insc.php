...
ini_set( 'error_reporting', E_ALL);
	...
	if (isset($_SESSION['uemail']) && isset($_SESSION['ulevel'])) {
		...
		if($_SESSION['ulevel'] >= 2) {
			...
			if($resdb != NULL && isset($_POST['cid'])) {

				$valExist = $dbHandler->isRegExistsMobile($resdb['id'], $_POST['cid']);
				
				if($valExist) {
					$resdb = $dbHandler->deleteRegMobile($resdb['id'], $_POST['cid']);
					...
					if($valFull) {
						$response['error'] = TRUE;
						$response['error_message'] = "No vacancies at this time";
					}else{
						$response['ttt'] = "before inserting".$resdb['id']."-".$_POST['cid'];
						$resdb = $dbHandler->insertRegMobile($resdb['id'], $_POST['cid']);
						$response['error'] = $resdb['error'];
						if($resdb['error']) $response['error_message'] = $resdb['error_message'];
					}
				}
